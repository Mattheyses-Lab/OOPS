function UpdateOrderLineScan(source)
...
%% create or update the rectangular patch representing the scan area
    ...
    if isvalid(OOPSData.Handles.LineScanRectangle) && isa(OOPSData.Handles.LineScanRectangle,'matlab.graphics.primitive.Patch')
        ...
        OOPSData.Handles.LineScanRectangle = patch(OOPSData.Handles.OrderAxH,...
            'XData',rectangleXData,...
            'YData',rectangleYData,...
            'FaceColor','yellow',...
            'EdgeColor','none',...
...
%% compute the 2D line scan
    ...
    LineScanMatrix = cell2mat(LineScanYData(1:nLines));

    % matrix of indices with non-zero values
    ...
    % compute the average over the non-zero values
...
%% plot the line scan
    ...
    plot(OOPSData.Handles.LineScanAxes,umXData,AverageLineScan,'Color',[0 0 0],'LineWidth',2);
