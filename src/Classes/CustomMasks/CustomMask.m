    ...
    methods
        ...
        function ExecuteStep(obj,StepIdx)
            try
                ...
                obj.Images(StepIdx).ImageData = zeros(size(obj.Operations(StepIdx).Target(1).ImageData));
                error('CustomMask:invalidOperationParameters',...
                    ['Unable to execute Operation ',num2str(StepIdx),'\n',ME.message]);
            end
        end
