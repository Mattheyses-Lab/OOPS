function [shape, I] = midlineCurvature2(...
    image,...
    flankingPoints,...
    curvatureThresh,...
    bp_tangent,...
...
%% Interpolate the curve for more points

% Interpolate for more points
% interpolation will be approximate to ensure points are evenly spaced along the entire curve

% interparc method
% get the length of the curve
curveLength = getCurveLength([x,y]);
% determine the number of edges to draw in the interpolated curve
nEdgesDesired = ceil(curveLength/interp_resolution);
% adjust interpolation resolution to ensure even spacing between all points
length1 = nEdgesDesired*interp_resolution;
length2 = curveLength;
interpScale = length1/length2;
interp_resolution = interp_resolution/interpScale;
% determine how many points in the final interpolated curve
nPointsDesired = round(curveLength/interp_resolution)+1;
% now interpolate the curve
interpCurve = interparc(nPointsDesired,x,y,'linear');
% get x and y coordinates of the new curve
xi = interpCurve(:,1);
yi = interpCurve(:,2);
...
windowWidth = round((flankingPoints)/interp_resolution)+1;
...
% re-interpolate for even spacing after the smoothing, keep same number of points
reinterpCurve = interparc(numel(smoothX), smoothX, smoothY, 'linear');
...
% calculate the curvature (by finding the radius of the oscillating circle using three neaby boundary points)

bp_positions = [shape_XY(end-flankingPoints:end-1,:); shape_XY(1:end,:); shape_XY(2:flankingPoints+1,:)];

parfor j = flankingPoints+1:M+flankingPoints
...
%     point3 = bp_positions(j+2*boundaryPoint,:); 
    ...
    point1 = bp_positions(j-flankingPoints,:);
    point2 = bp_positions(j,:);
    point3 = bp_positions(j+flankingPoints,:);    
    ...
    if slope12==slope23  
        shape_curvature(1,j-flankingPoints) = 0;
    ...
    % if the boundary is curved
        ...
        shape_uncutCurvature(1,j-flankingPoints) = 1/sqrt((point1(1,1)-x_center)^2+(point1(1,2)-y_center)^2);

        % cutoff the curvature (for visualization)
        shape_curvature(1,j-flankingPoints) = shape_uncutCurvature(1,j-flankingPoints);
        if shape_curvature(1,j-flankingPoints) > curvatureThresh
            warning('Detected curvature exceeding maximum')
            shape_curvature(1,j-flankingPoints) = curvatureThresh;
        end
...
%         end
        ...
        if ~isfinite(shape_uncutCurvature(1,j-flankingPoints))
            shape_curvature(1,j-flankingPoints) = 0;
            shape_uncutCurvature(1,j-flankingPoints) = 0;
        end
...
if ~loopclose
    ...
    curv_val = shape_curvature(flankingPoints+1);
    curv_interp = linspace(0,curv_val,flankingPoints);
    shape_curvature(1:flankingPoints) = curv_interp;

    % adjust end of curve
    curv_val = shape_curvature(end-flankingPoints);
    curv_interp = linspace(curv_val,0,flankingPoints);
    shape_curvature(end-flankingPoints+1:end) = curv_interp;
end
